...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing manual driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual manual: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var modeDevice = getModeDevice( device_data.id );
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    if (onoff) {
        console.log( "Turning on  " + modeDevice.data.id);

        Homey.manager('flow').triggerDevice('virtual_manual_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + modeDevice.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_manual_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_manual', function( callback, args ){
    var modeDevice = getModeDevice( args.device.id );
    if( modeDevice instanceof Error ) return callback( modeDevice );

    callback( null, modeDevice.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_manual_action_on', function( callback, args ){
    var modeDevice = getModeDevice( args.device.id );
    if( modeDevice instanceof Error ) return callback( modeDevice );

    modeDevice.state.onoff = true;
    var state = modeDevice.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_manual_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_manual_action_off', function( callback, args ){
    var modeDevice = getModeDevice( args.device.id );
    if( modeDevice instanceof Error ) return callback( modeDevice );

    modeDevice.state.onoff = false;
    var state = modeDevice.state;
    var tokens = {"type": "device"};
...
function getModeDevice( device_id ) {
    var device = devices[ device_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual Mode Device " + device_id);
    } else {
        return device;
    }
}
