...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing sleep driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual sleep: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualsleep = getsleep( device_data.id );
    if( virtualsleep instanceof Error ) return callback( virtualsleep );

    // send the state value to Homey
    callback( null, virtualsleep.state.onoff );
};

// this function is called by Homey when it wants to SET the sleepes state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualsleep.state.onoff = onoff;
    var state = virtualsleep.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualsleep.data.id);

        Homey.manager('flow').triggerDevice('virtual_sleep_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualsleep.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_sleep_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_sleep', function( callback, args ){
    var virtualsleep = getsleep( args.device.id );
    if( virtualsleep instanceof Error ) return callback( virtualsleep );

    callback( null, virtualsleep.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_sleep_action_on', function( callback, args ){
    var virtualsleep = getsleep( args.device.id );
    if( virtualsleep instanceof Error ) return callback( virtualsleep );

    virtualsleep.state.onoff = true;
    var state = virtualsleep.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_sleep_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_sleep_action_off', function( callback, args ){
    var virtualsleep = getsleep( args.device.id );
    if( virtualsleep instanceof Error ) return callback( virtualsleep );

    virtualsleep.state.onoff = false;
    var state = virtualsleep.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_sleep_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a sleep from the devices list by it's id
function getsleep( sleep_id ) {
    var device = devices[ sleep_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual sleep " + sleep_id);
    } else {
        return device;
    }
}

//a helper method to add a sleep to the devices list
