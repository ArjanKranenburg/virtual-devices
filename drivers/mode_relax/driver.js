...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing relax driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual relax: " + device_data.data.id);
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    if (onoff) {
        ...
        Homey.manager('flow').triggerDevice('virtual_relax_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + modeDevice.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_relax_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_relax', function( callback, args ){
...
Homey.manager('flow').on('action.virtual_relax_action_on', function( callback, args ){
    var modeDevice = getModeDevice( args.device.id );
    if( modeDevice instanceof Error ) return callback( modeDevice );
    ...
    Homey.manager('flow').triggerDevice('virtual_relax_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_relax_action_off', function( callback, args ){
    var modeDevice = getModeDevice( args.device.id );
    if( modeDevice instanceof Error ) return callback( modeDevice );
