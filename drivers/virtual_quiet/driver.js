...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing quiet driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual quiet: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualquiet = getquiet( device_data.id );
    if( virtualquiet instanceof Error ) return callback( virtualquiet );

    // send the state value to Homey
    callback( null, virtualquiet.state.onoff );
};

// this function is called by Homey when it wants to SET the quietes state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualquiet.state.onoff = onoff;
    var state = virtualquiet.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualquiet.data.id);

        Homey.manager('flow').triggerDevice('virtual_quiet_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualquiet.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_quiet_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_quiet', function( callback, args ){
    var virtualquiet = getquiet( args.device.id );
    if( virtualquiet instanceof Error ) return callback( virtualquiet );

    callback( null, virtualquiet.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_quiet_action_on', function( callback, args ){
    var virtualquiet = getquiet( args.device.id );
    if( virtualquiet instanceof Error ) return callback( virtualquiet );

    virtualquiet.state.onoff = true;
    var state = virtualquiet.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_quiet_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_quiet_action_off', function( callback, args ){
    var virtualquiet = getquiet( args.device.id );
    if( virtualquiet instanceof Error ) return callback( virtualquiet );

    virtualquiet.state.onoff = false;
    var state = virtualquiet.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_quiet_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a quiet from the devices list by it's id
function getquiet( quiet_id ) {
    var device = devices[ quiet_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual quiet " + quiet_id);
    } else {
        return device;
    }
}

//a helper method to add a quiet to the devices list
