...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing security driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual Security: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualsecurity = getsecurity( device_data.id );
    if( virtualsecurity instanceof Error ) return callback( virtualsecurity );

    // send the state value to Homey
    callback( null, virtualsecurity.state.onoff );
};

// this function is called by Homey when it wants to SET the securityes state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualsecurity.state.onoff = onoff;
    var state = virtualsecurity.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualsecurity.data.id);

        Homey.manager('flow').triggerDevice('virtual_security_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualsecurity.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_security_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_security', function( callback, args ){
    var virtualsecurity = getsecurity( args.device.id );
    if( virtualsecurity instanceof Error ) return callback( virtualsecurity );

    callback( null, virtualsecurity.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_security_action_on', function( callback, args ){
    var virtualsecurity = getsecurity( args.device.id );
    if( virtualsecurity instanceof Error ) return callback( virtualsecurity );

    virtualsecurity.state.onoff = true;
    var state = virtualsecurity.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_security_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_security_action_off', function( callback, args ){
    var virtualsecurity = getsecurity( args.device.id );
    if( virtualsecurity instanceof Error ) return callback( virtualsecurity );

    virtualsecurity.state.onoff = false;
    var state = virtualsecurity.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_security_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a security from the devices list by it's id
function getsecurity( security_id ) {
    var device = devices[ security_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual Security " + security_id);
    } else {
        return device;
    }
}

//a helper method to add a security to the devices list
