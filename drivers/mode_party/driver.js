...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing party driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual party: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualparty = getparty( device_data.id );
    if( virtualparty instanceof Error ) return callback( virtualparty );

    // send the state value to Homey
    callback( null, virtualparty.state.onoff );
};
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualparty.state.onoff = onoff;
    var state = virtualparty.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualparty.data.id);

        Homey.manager('flow').triggerDevice('virtual_party_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualparty.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_party_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_party', function( callback, args ){
    var virtualparty = getparty( args.device.id );
    if( virtualparty instanceof Error ) return callback( virtualparty );

    callback( null, virtualparty.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_party_action_on', function( callback, args ){
    var virtualparty = getparty( args.device.id );
    if( virtualparty instanceof Error ) return callback( virtualparty );

    virtualparty.state.onoff = true;
    var state = virtualparty.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_party_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_party_action_off', function( callback, args ){
    var virtualparty = getparty( args.device.id );
    if( virtualparty instanceof Error ) return callback( virtualparty );

    virtualparty.state.onoff = false;
    var state = virtualparty.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_party_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
function getparty( party_id ) {
    var device = devices[ party_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual party " + party_id);
    } else {
        return device;
    }
}
