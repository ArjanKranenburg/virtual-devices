...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    if( virtualaway instanceof Error ) return callback( virtualaway );
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    if (onoff) {
        ...
        Homey.manager('flow').triggerDevice('virtual_away_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualaway.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_away_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_away', function( callback, args ){
...
Homey.manager('flow').on('action.virtual_away_action_on', function( callback, args ){
    var virtualaway = getaway( args.device.id );
    if( virtualaway instanceof Error ) return callback( virtualaway );
    ...
    Homey.manager('flow').triggerDevice('virtual_away_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_away_action_off', function( callback, args ){
    var virtualaway = getaway( args.device.id );
    if( virtualaway instanceof Error ) return callback( virtualaway );
    ...
    Homey.manager('flow').triggerDevice('virtual_away_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
