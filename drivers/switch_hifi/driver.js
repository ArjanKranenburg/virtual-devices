...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing hifi driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual hifi: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualhifi = gethifi( device_data.id );
    if( virtualhifi instanceof Error ) return callback( virtualhifi );

    // send the state value to Homey
    callback( null, virtualhifi.state.onoff );
};

// this function is called by Homey when it wants to SET the hifies state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualhifi.state.onoff = onoff;
    var state = virtualhifi.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualhifi.data.id);

        Homey.manager('flow').triggerDevice('virtual_hifi_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualhifi.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_hifi_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_hifi', function( callback, args ){
    var virtualhifi = gethifi( args.device.id );
    if( virtualhifi instanceof Error ) return callback( virtualhifi );

    callback( null, virtualhifi.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_hifi_action_on', function( callback, args ){
    var virtualhifi = gethifi( args.device.id );
    if( virtualhifi instanceof Error ) return callback( virtualhifi );

    virtualhifi.state.onoff = true;
    var state = virtualhifi.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_hifi_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_hifi_action_off', function( callback, args ){
    var virtualhifi = gethifi( args.device.id );
    if( virtualhifi instanceof Error ) return callback( virtualhifi );

    virtualhifi.state.onoff = false;
    var state = virtualhifi.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_hifi_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a hifi from the devices list by it's id
function gethifi( hifi_id ) {
    var device = devices[ hifi_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual hifi " + hifi_id);
    } else {
        return device;
    }
}

//a helper method to add a hifi to the devices list
