...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing alarm driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual alarm: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualalarm = getalarm( device_data.id );
    if( virtualalarm instanceof Error ) return callback( virtualalarm );

    // send the state value to Homey
    callback( null, virtualalarm.state.onoff );
};

// this function is called by Homey when it wants to SET the alarmes state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualalarm.state.onoff = onoff;
    var state = virtualalarm.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualalarm.data.id);

        Homey.manager('flow').triggerDevice('virtual_alarm_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualalarm.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_alarm_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_alarm', function( callback, args ){
    var virtualalarm = getalarm( args.device.id );
    if( virtualalarm instanceof Error ) return callback( virtualalarm );

    callback( null, virtualalarm.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_alarm_action_on', function( callback, args ){
    var virtualalarm = getalarm( args.device.id );
    if( virtualalarm instanceof Error ) return callback( virtualalarm );

    virtualalarm.state.onoff = true;
    var state = virtualalarm.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_alarm_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_alarm_action_off', function( callback, args ){
    var virtualalarm = getalarm( args.device.id );
    if( virtualalarm instanceof Error ) return callback( virtualalarm );

    virtualalarm.state.onoff = false;
    var state = virtualalarm.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_alarm_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a alarm from the devices list by it's id
function getalarm( alarm_id ) {
    var device = devices[ alarm_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual alarm " + alarm_id);
    } else {
        return device;
    }
}

//a helper method to add a alarm to the devices list
