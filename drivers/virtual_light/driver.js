...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing light driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        console.log("Added Virtual light: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtuallight = getlight( device_data.id );
    if( virtuallight instanceof Error ) return callback( virtuallight );

    // send the state value to Homey
    callback( null, virtuallight.state.onoff );
};

// this function is called by Homey when it wants to SET the lightes state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtuallight.state.onoff = onoff;
    var state = virtuallight.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtuallight.data.id);

        Homey.manager('flow').triggerDevice('virtual_light_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtuallight.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_light_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_light', function( callback, args ){
    var virtuallight = getlight( args.device.id );
    if( virtuallight instanceof Error ) return callback( virtuallight );

    callback( null, virtuallight.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_light_action_on', function( callback, args ){
    var virtuallight = getlight( args.device.id );
    if( virtuallight instanceof Error ) return callback( virtuallight );

    virtuallight.state.onoff = true;
    var state = virtuallight.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_light_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_light_action_off', function( callback, args ){
    var virtuallight = getlight( args.device.id );
    if( virtuallight instanceof Error ) return callback( virtuallight );

    virtuallight.state.onoff = false;
    var state = virtuallight.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_light_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a light from the devices list by it's id
function getlight( light_id ) {
    var device = devices[ light_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual light " + light_id);
    } else {
        return device;
    }
}

//a helper method to add a light to the devices list
