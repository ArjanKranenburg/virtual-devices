...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing light button driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        var device_data = {
            name: "New Virtual Light Button",
            data: {
                id: guid()
            }
        }

        console.log("Added Virtual Light Button: " + device_data.data.id);
...
function getSwitch( switch_id ) {
    ...
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual Light Button " + switch_id);
    } else {
        return device;
    }
}
