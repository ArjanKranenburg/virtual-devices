...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing holiday driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        var device_data = {
            name: "Virtual holiday",
            data: {
                id: guid()
            }
        }

        console.log("Added Virtual holiday: " + device_data.data.id);
...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    var virtualholiday = getholiday( device_data.id );
    if( virtualholiday instanceof Error ) return callback( virtualholiday );

    // send the state value to Homey
    callback( null, virtualholiday.state.onoff );
};

// this function is called by Homey when it wants to SET the holidayes state, e.g. when the user presses the button on
// the smartphone
// `device_data` is the object as saved during pairing
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    virtualholiday.state.onoff = onoff;
    var state = virtualholiday.state;
    var tokens = {"type": "device"};

    if (onoff) {
        console.log( "Turning on  " + virtualholiday.data.id);

        Homey.manager('flow').triggerDevice('virtual_holiday_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualholiday.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_holiday_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_holiday', function( callback, args ){
    var virtualholiday = getholiday( args.device.id );
    if( virtualholiday instanceof Error ) return callback( virtualholiday );

    callback( null, virtualholiday.state.onoff ); 
});

Homey.manager('flow').on('action.virtual_holiday_action_on', function( callback, args ){
    var virtualholiday = getholiday( args.device.id );
    if( virtualholiday instanceof Error ) return callback( virtualholiday );

    virtualholiday.state.onoff = true;
    var state = virtualholiday.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_holiday_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_holiday_action_off', function( callback, args ){
    var virtualholiday = getholiday( args.device.id );
    if( virtualholiday instanceof Error ) return callback( virtualholiday );

    virtualholiday.state.onoff = false;
    var state = virtualholiday.state;
    var tokens = {"type": "device"};

    Homey.manager('flow').triggerDevice('virtual_holiday_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
//a helper method to get a holiday from the devices list by it's id
function getholiday( holiday_id ) {
    var device = devices[ holiday_id ];
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual holiday " + holiday_id);
    } else {
        return device;
    }
}

//a helper method to add a holiday to the devices list
