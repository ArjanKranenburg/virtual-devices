...
module.exports.capabilities.onoff.get = function (device_data, callback) {
    ...
    if( virtualtv instanceof Error ) return callback( virtualtv );
...
module.exports.capabilities.onoff.set = function( device_data, onoff, callback ) {
    ...
    if (onoff) {
        ...
        Homey.manager('flow').triggerDevice('virtual_tv_on', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    } else {
        console.log("Turning off " + virtualtv.data.id);
    	
        Homey.manager('flow').triggerDevice('virtual_tv_off', tokens, state, device_data, function (err, result) {
       		if (err) return console.error(err);
    	});
    }
...
Homey.manager('flow').on('condition.virtual_tv', function( callback, args ){
...
Homey.manager('flow').on('action.virtual_tv_action_on', function( callback, args ){
    var virtualtv = gettv( args.device.id );
    if( virtualtv instanceof Error ) return callback( virtualtv );
    ...
    Homey.manager('flow').triggerDevice('virtual_tv_on', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
...
Homey.manager('flow').on('action.virtual_tv_action_off', function( callback, args ){
    var virtualtv = gettv( args.device.id );
    if( virtualtv instanceof Error ) return callback( virtualtv );
    ...
    Homey.manager('flow').triggerDevice('virtual_tv_off', tokens, state, args.device, function (err, result) {
   		if (err) return console.error(err);
	});
