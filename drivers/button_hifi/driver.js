...
module.exports.init = function( devices_data, callback ) {
    console.log("Initializing hifi button driver");
...
module.exports.pair = function( other ) {
	other.on('list_devices', function( data, callback ){
        ...
        var device_data = {
            name: "New Virtual Hi-Fi Button",
            data: {
                id: guid()
            }
        }

        console.log("Added Virtual Hi-Fi Button: " + device_data.data.id);
...
module.exports.capabilities.button.set = function( args, button, callback ) {
    ...
    Homey.manager('flow').triggerDevice('button_hifi_pushed', tokens, true, args, function (err, result) {
...
Homey.manager('flow').on('action.button_hifi_pushed', function( callback, args ){
    var virtualButton = getSwitch( args.id );
...
function getSwitch( switch_id ) {
    ...
    if( typeof device === 'undefined' ) {
        return new Error("Could not find Virtual Hi-Fi Button " + switch_id);
    } else {
        return device;
    }
}
